export const DETAIL_URL = "https://pokeapi.co/api/v2/pokemon/";
export const WORDPRESS_BASE_URL = "http://localhost:8080/wp-practice/wordpress/";
export const TOKEN_PATH = "wp-json/jwt-auth/v1/token";

//Unknown meltan & melmetal = https://pokeapi.co/api/v2/pokemon?limit=2&offset=807

//Kanto Generation I
export const KANTO_URL = "https://pokeapi.co/api/v2/pokemon?limit=151&offset=0";
//Johto Generation II
export const JOHTO_URL = "https://pokeapi.co/api/v2/pokemon?limit=100&offset=151";
//Hoenn Generation II
export const HOENN_URL = "https://pokeapi.co/api/v2/pokemon?limit=135&offset=251";
//Sinnoh Generation III
export const SINNOH_URL = "https://pokeapi.co/api/v2/pokemon?limit=107&offset=386";
//Unova Generation IV
export const UNOVA_URL = "https://pokeapi.co/api/v2/pokemon?limit=156&offset=493";
//Kalos Generation V
export const KALOS_URL = "https://pokeapi.co/api/v2/pokemon?limit=72&offset=649";
//Alola Generation VI
export const ALOLA_URL = "https://pokeapi.co/api/v2/pokemon?limit=86&offset=721";
//Galar Generation VII
export const GALAR_URL = "https://pokeapi.co/api/v2/pokemon?limit=89&offset=809";
//Hisui Generation VIII
export const HISUI_URL = "https://pokeapi.co/api/v2/pokemon?limit=7&offset=898";
//Paldea Generation XI
export const PALDEA_URL = "https://pokeapi.co/api/v2/pokemon?limit=105&offset=905";
