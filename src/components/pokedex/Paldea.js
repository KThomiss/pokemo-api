...
import { PALDEA_URL } from "../../constants/api";
...
export default function ListOfPokemon() {
  ...
  const { loading, error, data, setData, filteredData } = useApi(PALDEA_URL);
          ...
          return (
            <li key={index} className="pokemon-list">
              <Link to={`/detail/${list.name}`} className="pokemon-link">
                <img src={require(`../../images/official-artwork/paldea/${list.name}.png`)} alt={list.name} className="list-img" />
                {list.name}
              </Link>
            </li>
          );
